...
export default function CompliancePage() {
  ...
  function downloadReport() {
    ...
    let md = `# Spekris Compliance Report\n\n`;
    md += `**Generated:** ${new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}\n\n`;
    md += `---\n\n`;
    md += `## Executive Summary\n\n`;
    md += `**Overall Compliance Score: ${overall}%**\n\n`;
    md += `Spekris assessed ${frameworks.reduce((s, f) => s + f.total_controls, 0)} controls across ${frameworks.length} frameworks against your AI asset inventory.\n\n`;

    if (overall >= 80) {
      md += `Your organization demonstrates strong compliance posture across the assessed frameworks.\n\n`;
    } else if (overall >= 50) {
      md += `Your organization has moderate compliance coverage. Several controls require attention to improve your posture.\n\n`;
    } else {
      md += `Your organization has significant compliance gaps that should be addressed before your next audit.\n\n`;
    }

    md += `| Framework | Score | Compliant | Gaps |\n`;
    md += `| --- | --- | --- | --- |\n`;
    for (const fw of frameworks) {
      const meta = FRAMEWORKS.find((f) => f.code === fw.code);
      md += `| ${meta?.name ?? fw.code} | ${fw.score}% | ${fw.compliant_controls}/${fw.total_controls} | ${fw.non_compliant_controls} |\n`;
    }
    md += `\n`;
    ...
    for (const fw of frameworks) {
      const meta = FRAMEWORKS.find((f) => f.code === fw.code);
      md += `---\n\n## ${meta?.name ?? fw.code}\n\n`;
      md += `**Score: ${fw.score}%** — ${fw.compliant_controls} of ${fw.total_controls} controls compliant\n\n`;
      if (fw.non_compliant_controls > 0) {
        md += `**${fw.non_compliant_controls} gap${fw.non_compliant_controls === 1 ? "" : "s"} identified.**\n\n`;
      }
    }
    ...
    if (gaps.length > 0) {
      ...
      for (const gap of gaps) {
        md += `### ${gap.control_id} — ${gap.control_name}\n\n`;
        md += `- **Status:** ${gap.status.replace("_", " ")}\n`;
        md += `- **Framework:** ${gap.framework}\n`;
        md += `- **Affected assets:** ${gap.asset_count}\n\n`;
      }
    }

    md += `---\n\n`;
    md += `*This report was auto-generated by Spekris. It supports compliance documentation efforts and is not a substitute for a qualified auditor.*\n`;

    const blob = new Blob([md], { type: "text/markdown" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `spekris-compliance-report-${date}.md`;
    a.click();
